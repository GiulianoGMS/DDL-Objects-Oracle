CREATE OR REPLACE PROCEDURE NAGP_REP_CONTROLEDATAS_APP_PROMOC (psNroEmpresa NUMBER, psDataAtividade DATE) AS

-- Nao utilizar pois gera uma promoc por item
...
SELECT /*+OPTIMIZER_FEATURES_ENABLE('11.2.0.4')*/ 
       ...
       SELECT DISTINCT 
       M.NROEMPRESA, 'M' CENTRALLOJA, M.NROSEGMENTOPRINC NROSEGMENTO, 'CDV Avulsa'||LPAD(M.NROEMPRESA,2,0)||' - Ate '||TO_CHAR(A.DATA_VALIDADE - 1, 'DD/MM/YYYY')||' - '||SUBSTR(P.DESCCOMPLETA,0,40) PROMOCAO,
...
-- Itens
  ...
  FROM NAGV_APP_DATAVALIDADE_CONTROLE_AVULSA@CONSINCODW A   INNER JOIN MAX_EMPRESA M ON M.NROEMPRESA = A.LOJA
...
WHERE DATA_ATIVIDADE = psDataAtividade
  AND C.CATEGORIAN1 = 'FRIOS E LATICINIOS' 
  AND LOJA = psNroEmpresa
  -- Remove dup
  AND NOT EXISTS (SELECT 1 FROM MRL_PROMOCAO PR WHERE PR.PROMOCAO = 'CDV Avulsa'||LPAD(M.NROEMPRESA,2,0)||' - Ate '||TO_CHAR(A.DATA_VALIDADE - 1, 'DD/MM/YYYY')||' - '||SUBSTR(P.DESCCOMPLETA,0,40))
  AND E.ESTQLOJA >= A.QTDE_REBAIXA
  ...
  LOOP
    ...
    INSERT INTO NAGT_CONTROLE_VALIDADE_INS@CONSINCODW (NROEMPRESA, SEQPRODUTO, TIPO, DATA_REG, DATA_ATIV, DATA_VAL, SEQPROMOCAO)
     ...
     UPDATE NAGT_REBAIXA_AVULSA@CONSINCODW R 
                                        SET R.IND_PROCESSADO = 'S'
                                      WHERE R.IND_PROCESSADO = 'N'
                                        AND R.NROEMPRESA = promoc.Nroempresa
                                        AND R.SEQPRODUTO = promoc.SEQPRODUTO;   
    END IF; 
    END LOOP;
