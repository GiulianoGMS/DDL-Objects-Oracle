...
BEGIN
    ...
    SELECT COUNT(DISTINCT A.NRO_ACORDO), MAX(A.FORNECEDOR), MAX(INITCAP(COMPRADOR)), MAX(INITCAP(A.RESPACORDONOME))
     ...
     WHERE A.EMAIL = psEmail
       ...
       AND NOT EXISTS (SELECT 1 FROM NAGT_LOG_ENVIO_ACO_EMAIL E WHERE E.EMAIL_DESTINO = psEmailTI||psEmail AND TRUNC(E.DATA_ENVIO) = TRUNC(SYSDATE));
